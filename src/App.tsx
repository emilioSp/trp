							...
							<table className="w-full table-auto text-left">
								...
								<tbody className="text-gray-700">
									{iRatingRank.map((m, idx) => (
										<tr
											key={m.id}
											className="border-b border-gray-100 even:bg-indigo-50/30 hover:bg-indigo-50 transition-colors"
										>
											<td className="px-3 py-2 font-medium w-12">{idx + 1}</td>
											<td className="px-3 py-2">{m.name}</td>
											<td className="px-3 py-2 text-right">
												<span
													className={`inline-block px-2 py-0.5 rounded-full ${getClassNameForSR(m.className)} text-sm font-semibold`}
												>
													{m.iRating}
												</span>
											</td>
